...
for dataset in datasets:
	...
	for i in range(k_fold):
		...
		LR = logistic.logistic_regression(train_x.shape[1], train_y.shape[1], max_iter=1000,thres=0.05,lr=0.0001)
...
axes[0,0].plot(train_accs_avg[0],label="train set")
axes[0,0].plot(test_accs_avg[0],label='validation set')
axes[0,0].legend(['train set','validation set'])
